<template>
    <div class="field">
        <div class="lines">
                <FieldLine v-for="(field,index) in fieldArranged" :key="field" :index="index" :type="field.type" :side="field.side" :cards="field.cards" />
        </div>
    </div>
</template>
        ...
        ...mapState('board', {
            fieldArranged(state){
                ...
                if(state.player == 2){
                    ...
                    // Reverse mutate the original state ... so i had to use slice to copy it efficientlys
