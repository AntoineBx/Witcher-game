...
import { ADD_CARD_TO_FIELD,UPDATE_CARD_STATE_COORD } from '@/store/modules/board/mutations'
      ...
      '$store.state.board.mousePosOnRelease': function() {
          ...
          if(this.allowDroping != undefined){
            ...
            if(lenghtX.left <= this.$store.state.board.mousePosOnRelease.x && this.$store.state.board.mousePosOnRelease.x <= lenghtX.right &&
              ...
              console.log({idCard : this.wasHolding , row : newRow});
              console.log("placeCardOnField ! !");
              //Temporary lock card on place waiting for the server response that will change coord
              this[UPDATE_CARD_STATE_COORD]({idCard : this.wasHolding, coord : {x : this.$store.state.board.mousePosOnRelease.x, y:this.$store.state.board.mousePosOnRelease.y}})
