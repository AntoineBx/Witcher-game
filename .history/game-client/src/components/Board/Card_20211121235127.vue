...
<div class="placeholder" :class="[holdingClass ?'isHolding':'' , statusClass]" ref="placeholder" :Width="CardWidth" :Height="CardHeight" >
    <div class="card-container" ref="currentContainer" :Width="CardWidth" :Height="CardHeight"  >
        ...
        <div class="card" ref="currentCard" @mousedown="triggerHoldClick" @mouseup="triggerReleaseClick">
            ...
            <div class="video">
                <video ref="videoRef"  id="video-container" :src="urlVideo" @loadedmetadata="onVideoLoad"  loop muted Width="350px" Height="500px">
...
import { OPEN_POPUP } from "@/store/modules/popupCard/mutations";
    ...
    export default {
        ...
        watch: {
            '$store.state.board.triggercardsCoord': function() {
                if( this.$store.state.board.cardsState[this.uuid] == undefined ) return;
                this.StoreLockPosition = this.$store.state.board.cardsState[this.uuid].coord
                this.status = this.$store.state.board.cardsState[this.uuid].status
            },
            '$store.state.board.playerTurn': function() {
                if(this.$store.state.board.playerTurn == this.$store.state.board.player && !this.unmovable){
                    var index = this.status.indexOf('isUnmovable');
                    if (index !== -1) {
                        this.status.splice(index, 1);
                    }
                    this.status.push('isUnmovable');
                }else{
                    this.status.push('isUnmovable');
                }
            ...
            //transform coord on the page to translate() coord
            StoreLockPosition(){
        ...
        computed: {
            ...
            powerImg : function(){
              return "/src/assets/misc/number/"+this.CardData.power+".png";
            },
        ...
        created: function(){
            this[UPDATE_CARD_STATE_COORD]({ uuid : this.uuid,coord:{x:0,y:0}})
            this.SetCardData();
            
            
            this.isUnmovable = this.unmovable
        },
        mounted: function(){
        ...
        methods: {
            ...
            SetCardData:function(){
                ...
                if(currentCard == undefined){
                    console.error("Props Card name not found in the Json ")
                    return;
                }
...
//------------------//
                ...
                if(!this.isUnmovable && !this.status?.includes('onField') ){
                    ...
                    if(event.which == 1){
                        if(!this.hidden){
                            ...
                            this[SET_HOLDING]({uuid : this.uuid, positioning : this.CardData.positioning });
                        }
                    }
                }
            },
            triggerReleaseClick:function(event){
                if(!this.isUnmovable){
                    ...
                    this[SET_WAS_HOLDING](this.uuid);
        ...
        beforeDestroy : function(){
            this[REMOVE_CARD_STATE](this.uuid)
        }
    }
