    ...
    watch: {
        syncPlaceholders: function() {
           setTimeout(() => {
                ...
                this.setCoord()
                this[ADD_CARD_STATUS]({idCard : this.card.uuid, status : "onField"})
           }, 100);
        }
    },
    methods: {
        ...
        setCoord(){
            let ref = this.$refs.currentPlaceholder
            this[UPDATE_CARD_STATE_COORD]({idCard : this.card.idCard, coord: { x: ref.getBoundingClientRect().left, y: ref.getBoundingClientRect().top}})
        }
    },
    mounted :function(){
        this.setCoord()
