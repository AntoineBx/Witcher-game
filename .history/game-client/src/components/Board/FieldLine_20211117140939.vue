<template>
  <div class="line">
    <div ref="currentLine"  class="inner-line" :class="[type , allowDrop != undefined ? 'highlight' : '']" >
...
import { ADD_PLACEHOLDER_TO_FIELD, UPDATE_CARD_STATE_COORD, } from '@/store/modules/board/mutations'
      ...
      '$store.state.board.mousePosOnRelease': function() {
          ...
          if(this.allowDroping != undefined){
            if(lenghtX.left <= this.$store.state.board.mousePosOnRelease.x && this.$store.state.board.mousePosOnRelease.x <= lenghtX.right &&
              ...
              this[ADD_PLACEHOLDER_TO_FIELD]({uuid : this.wasHolding ,Row :this.index, position : 1})
