...
export default {
    ...
    [UPDATE_CARD_STATE_COORD](state,{idCard,coord}){
        ...
        console.log(UPDATE_CARD_STATE_COORD);
        let xx = coord.x;
        let yy = coord.y;
        if(state.cardsState[idCard]?.coord != undefined){
    ...
    [ADD_CARD_STATUS](state,{idCard,status}){
        if(state.cardsState[idCard]?.coord != undefined){
            if(state.cardsState[idCard]?.status != undefined){//check if there is any status in array 
