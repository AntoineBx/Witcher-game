...
globalThis.io.on("connection", function (socket) {
  ...
  socket.on("cardsPicked", handleMoveCards);
...
function handleMoveCards({player,idCards,fromWhere,toWhere,numberOfCards}){
  console.log("handleMoveCards",{player,idCards,fromWhere,toWhere,numberOfCards})
  if(this.roomName){
    games[this.roomName].moveCards(player,idCards,fromWhere,toWhere,numberOfCards);
  }else{
    console.error("Error : RoomName is expired")
  }
}
