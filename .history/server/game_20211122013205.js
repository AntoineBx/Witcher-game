...
class Game {
  ...
  addPlayer(socket, username, deck) {
    ...
    this.players.push({socket : socket.id, player : new Player(socket.id, username, deck, playerPosition) });
  ...
  start() {
    ...
    let deck1 = [];
    let deck2 = [];
    console.log(this.players.deck);
    this.players.forEach((player,index) =>{
      console.log(player.player.deck);
      if(index == 0) {
        deck1 = player.player.deck;
      } 
      if(index == 1) {
        deck2 = player.player.deck;
      } 
    })
    this.board = new Board(deck1,deck2);
    this.send("BoardUpdate", {board : this.board});
    this.send("start")
    this.send("waiting", {waiting : false, time:0 , maxPlayer : true});
  }
  move(cellId){
    console.log("MOVED : ",cellId)
  }
