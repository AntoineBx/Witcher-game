...
class Game {
  ...
  send( eventName, data=null, socketId=undefined ,broadcast){
  ...
  addPlayer(socket, username, deck){
    ...
    this.send("playerPosition", playerPosition, socket.id,false)
  ...
  placeCardOnField(socket,idCard,row,position){
    console.log("placeCardOnField", socket);
