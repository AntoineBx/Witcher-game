...
class Game {
  ...
  preStart() {
    ...
    this.send("SetAllCards", this.board.cards);
  ...
  placeCardOnField(uuid,row,position) {
    let foundIn = this.board.findCard(uuid);
    console.log("foundIn", foundIn);
    this.board.removeCard(foundIn.element,foundIn.row,foundIn.position)
